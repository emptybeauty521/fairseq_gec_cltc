...
class TransformerModel(FairseqModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--use-encoder-classification', default=False, action='store_true',
                            help='whether to use token-level labeling task')
        parser.add_argument('--use-sentence-copying', default=False, action='store_true',
                            help='whether to use sentence-level copying task')
        # fmt: on
...
class TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, left_pad=True):
        ...
        self.use_encoder_classification = args.use_encoder_classification
        if self.use_encoder_classification:
            self.encoder_cls_linear = nn.Linear(args.encoder_embed_dim, 2)

    def forward(self, src_tokens, src_lengths, err_flag=None):
        """
        Args:
        ...
        if self.training and self.use_encoder_classification:
            x = x.transpose(0, 1)
            src_logits = self.encoder_cls_linear(x)
            x = x.transpose(0, 1)
        else:
            src_logits = None
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def forward(self, prev_output_tokens, encoder_out=None, incremental_state=None):
        ...
        return x, {'attn': attn, 'inner_states': inner_states, 
            ...
            'src_logits': encoder_out['src_logits']}
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False):
        ...
        self.use_sentence_copying = args.use_sentence_copying
    ...
    def forward(self, x, encoder_out, encoder_padding_mask, incremental_state,
                ...
                self_attn_padding_mask=None, err_flag=None):
        """
        Args:
            x (Tensor): input to the layer of shape `(seq_len, batch, embed_dim)`
            encoder_padding_mask (ByteTensor): binary ByteTensor of shape
