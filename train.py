...
def main(args, init_distributed=False):
    ...
    # 加载第1个批次训练数据，生成1个伪batch
    task.load_dataset(args.train_subset, combine=False)
    # 加载验证集
    load_validsets(task)
...
def load_trainsets(task):
    """逐个加载训练集"""
    for k in itertools.count():
        trainset_k = "train" + str(k + 1) + "_0"
        try:
            task.load_dataset(trainset_k, combine=False)
            yield trainset_k
...
def load_validsets(task):
    """加载所有验证集"""
    for k in itertools.count():
        validset_k = "valid" + (str(k) if k > 0 else '')
        try:
            task.load_dataset(validset_k, combine=False)
        except FileNotFoundError as e:
            if k > 0:
                break
            raise e
